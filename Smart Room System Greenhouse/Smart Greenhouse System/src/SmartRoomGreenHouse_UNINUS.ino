...
#define BUTTON_S1 8     // Tombol Blower
...
// // Buffer sizes
// #define MAX_MODBUS_DEVICES 6
// #define TOTAL_SOURCES (MAX_MODBUS_DEVICES + 1) // 6 Modbus devices + 1 SCD30

// // Buffers for temperature and humidity
// float temperatureBuffer[MAX_MODBUS_DEVICES] = {0};
// float humidityBuffer[MAX_MODBUS_DEVICES] = {0};
...
void publishData() {
    ...
    //     String suhuKey = "suhu" + String(i + 1);
    //     String humKey = "kelembaban" + String(i + 1);
    //     jsonDoc[suhuKey] = temperatureBuffer[i];
    //     jsonDoc[humKey] = humidityBuffer[i];
    // }

      // Tambahkan data suhu dan kelembaban dari setiap sensor
    jsonDoc["suhu1"] = temp1;
...
//         temperatureBuffer[slaveID - 1] = temperature;
//         humidityBuffer[slaveID - 1] = humidity;
//     }
// }
...
void setup() {
    ...
    xTaskCreate(blowertask, "Blower Task", 2048, NULL, 1, NULL);
    ...
    xTaskCreatePinnedToCore(displayDataOnLCD, "display LCD", 4096, NULL,1,NULL,1);
}
...
void loop() {
    ...
    // for (int i = 0; i < MAX_MODBUS_DEVICES; i++) {
    //     avgTemp += temperatureBuffer[i];
    //     avgHum += humidityBuffer[i];
    // }

    // if (MAX_MODBUS_DEVICES > 0) {
    //     avgTemp = (avgTemp + scd30Temperature) / (MAX_MODBUS_DEVICES + 1);
    //     avgHum = (avgHum + scd30Humidity) / (MAX_MODBUS_DEVICES + 1);
    // }
